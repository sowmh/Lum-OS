...
def smoke_test() -> None:
    ...
    print("[build] running smoke test")
    ...
    expected = [
        "Commands: help, about, clear, mem, boot, irq, ticks, uptime, echo <text>, reboot, halt",
        "Interrupt status:",
        "Uptime:",
        "Conventional memory:",
        "CPU halted.",
    ...
    if missing:
        raise RuntimeError(f"Smoke test failed. Missing markers: {missing}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "target",
        choices=["all", "image", "stage1", "stage2", "kernel", "run", "run-headless", "smoke-test", "clean"],
        nargs="?",
        default="all",
    )
