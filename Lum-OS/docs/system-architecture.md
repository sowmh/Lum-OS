...
- loads an IDT covering CPU exceptions and remapped IRQs
- remaps the PIC and runs the PIT at 100 Hz
- receives keyboard input through IRQ1 into a ring buffer
- keeps serial input available for headless testing
- exposes a minimal command shell with runtime status commands
...
- visible interrupt infrastructure inside the kernel
