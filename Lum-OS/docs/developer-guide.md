...
- `src/kernel/kernel.asm`
  ...
  - PIC remap and PIT initialization
  - line input from serial and IRQ-driven keyboard buffering
  - command shell
...
- the kernel banner
- interrupt subsystem status lines
- the `lum>` prompt
...
- Extend the interrupt layer with more IRQ handlers, drivers, and recovery paths
